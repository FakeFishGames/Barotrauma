            ...
            foreach ((Affliction affliction, LimbHealth limbHealth) in afflictions)
            {
                ...
                if (affliction.Strength > 0.0f && affliction.Strength >= affliction.Prefab.ActivationThreshold && currentEffect != null && currentEffect.ConvulseAmount > 0f)
                {
                    foreach (Limb limb in Character.AnimController.Limbs)
                    {
                        ...
                        float force = Rand.Value() * 0.5f * limb.Mass * currentEffect.ConvulseAmount;
                        limb.body.ApplyLinearImpulse(Rand.Vector(force), maxVelocity: NetConfig.MaxPhysicsBodyVelocity * 0.5f);
                    }
                }
            }
#if CLIENT
