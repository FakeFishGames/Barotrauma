            ...
            void addRegularModToList(RegularPackage mod, GUIListBox list)
            {
                ...
                var contextMenuHandler = new GUICustomComponent(new RectTransform(Vector2.Zero, modFrame.RectTransform),
                    onUpdate: (f, component) =>
                    {
                        ...
                        contextMenuOptions.Add(new(swapLabel, true, currentSwapFunc ?? NoOp));

                        if (ContentPackageManager.WorkshopPackages.Contains(mod))
                        {
                            if (selectedMods.Length == 1)
                            {
                                contextMenuOptions.Add(new("ViewWorkshopModDetails".ToIdentifier(), true, () => PrepareToShowModInfo(mod)));

                                contextMenuOptions.Add(new("CopyWorkshopToLocal".ToIdentifier(), true, () => CopyToLocal()));
                            }
                        ...
                        else if (ContentPackageManager.LocalPackages.Contains(mod))
                        {
                            if (selectedMods.Length == 1)
                            {
                                contextMenuOptions.Add(new ContextMenuOption("RenamePackage".ToIdentifier(), true, AskToRenameLocal));
                            }
                            ...
                            if (selectedMods.All(ContentPackageManager.LocalPackages.Contains))
                            {
                                if (selectedMods.Length > 1)
                                {
                                    contextMenuOptions.Add(new("MergeSelectedMods".ToIdentifier(), true, () => ModMerger.AskMerge(selectedMods)));
                                }

                                contextMenuOptions.Add(new("Delete".ToIdentifier(), true, AskToDeleteLocal));
                            }
                        }
                        ...
                        {
                            GUIMessageBox msgBox = new(TextManager.Get("RenamePackage") + $" {mod.Name}", "", new LocalizedString[] { TextManager.Get("Cancel"), TextManager.Get("Confirm") });
                        ...
                        {
                            GUIMessageBox msgBox = new(TextManager.Get("DeleteMods"), TextManager.GetWithVariables("DeleteModsConfirm", ("[amount]", selectedMods.Length.ToString()), ("[plural]", selectedMods.Length == 1 ? "" : "s")), new LocalizedString[] { TextManager.Get("Cancel"), TextManager.Get("Confirm") }, textAlignment: Alignment.TopCenter);
                            msgBox.Buttons[0].OnClicked += msgBox.Close;
                            msgBox.Buttons[1].OnClicked += (_, _) =>
                            {
