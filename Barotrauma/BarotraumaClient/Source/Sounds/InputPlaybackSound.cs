...
namespace Barotrauma.Sounds
{
    public class InputPlaybackSound : Sound
    {
        ...
        private const int FREQUENCY = 24000; //not amazing, but not bad audio quality
        private const int BUFFER_SIZE = 1440; //60ms window, the max Opus seems to support
        private const int REPEAT_MARGIN = 720; //30ms window of audio to repeat in case not enough audio is available
        ...
        public InputPlaybackSound(SoundManager owner) : base(owner, "input", true)
        {
            ...
            //set up Opus
            encoder = new OpusEncoder(24000, 1, OpusApplication.OPUS_APPLICATION_VOIP);
        ...
        public override int FillStreamBuffer(int samplePos, short[] buffer)
        {
            ...
            int compressedCount = encoder.Encode(buffer, 0, BUFFER_SIZE, compressed, 0, BUFFER_SIZE);
            //Console.WriteLine("UNCOMPRESSED: " + BUFFER_SIZE.ToString() + "; COMPRESSED: " + compressedCount.ToString());
            decoder.Decode(compressed, 0, compressedCount, buffer, 0, BUFFER_SIZE);
